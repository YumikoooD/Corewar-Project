...
void execute_ld_operation(fighter_t *fighter, int arg1, int reg)
{
...
int cmd_ld(vm_t *data, fighter_t *fighter)
{
    ...
    int arg1 = get_arg_value_by_type_relative(data, arg1_type,
        address + 2, fighter);
    int reg = data->arena[normalize_adress(NULL, address + 2 +
            type_size(arg1_type))];
    args_type_t args[] = {arg1_type, 1};
    execute_ld_operation(fighter, arg1, reg);
